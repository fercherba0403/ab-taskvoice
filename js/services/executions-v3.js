...
export async function uploadExecutionReportAudio(
    ...
    if (!Number.isSafeInteger(Number(executionId)) || Number(executionId) <= 0) {
        throw new Error("No se encontró una ejecución válida para el reporte.");
    }
...
export async function removeExecutionReportAudio(audioPath) {
    if (!audioPath) {
        return;
    }
...
export async function addExecutionReport(
    ...
    if (!Number.isSafeInteger(reportId) || reportId <= 0) {
        throw new Error("Supabase no devolvió un reporte válido.");
    }
...
export async function getExecutionReports(executionId) {
    if (!executionId) {
        return [];
    }
...
export async function createExecutionReportAudioSignedUrl(audioPath) {
    return createExecutionAudioSignedUrl(audioPath);
}
...
export async function transcribeExecutionReport(reportId) {
