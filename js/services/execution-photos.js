...
export function validateExecutionPhotoFile(file) {
    if (!file) {
        throw new Error("No se seleccionó ninguna fotografía.");
    }
    ...
    if (file.size > MAX_PHOTO_BYTES) {
        throw new Error("La fotografía supera el máximo de 10 MB.");
    }
}
...
function createPhotoFileName(file) {
    const extension = getPhotoExtension(file.type);
...
export async function getExecutionPhotos(executionId) {
...
export async function createExecutionPhotoSignedUrl(storagePath) {
    if (!storagePath) {
        return null;
    }
...
export async function uploadExecutionPhoto(task, executionId, file) {
    validateExecutionPhotoFile(file);

    if (!task?.organization_id || !task?.id || !executionId) {
        throw new Error("No se pudo determinar la ejecución de la fotografía.");
    }

    const fileName = createPhotoFileName(file);
    ...
    const { error: uploadError } = await supabase.storage.from("fotos").upload(
        storagePath,
...
export async function removeExecutionPhoto(photo) {
    if (!photo?.id || !photo?.storage_path) {
        throw new Error("Fotografía inválida.");
    }
