...
function getActivePage() {
    return document.body.dataset.technicianPage || "tasks";
}
...
function getInitials(fullName) {
    ...
    return `${firstInitial}${lastInitial}`.toLocaleUpperCase("es-AR");
}

function bindUserPresentation() {
    const userName = document.getElementById("topbarUserName");

    const avatar = document.getElementById("technicianUserAvatar");
    ...
    const updateAvatar = () => {
        avatar.textContent = getInitials(userName.textContent ?? "");
    };
...
function ensureOverlay() {
    let overlay = document.getElementById("technicianSidebarOverlay");
    ...
    overlay.id = "technicianSidebarOverlay";

    overlay.className = "technician-sidebar-overlay";
...
function ensureMenuButton() {
    const topbarLeft = document.querySelector(".technician-topbar-left");

    if (!topbarLeft) {
        return null;
    }

    let button = document.getElementById("technicianMenuButton");
    ...
    button.id = "technicianMenuButton";

    button.className = "technician-menu-button";

    button.type = "button";

    button.setAttribute("aria-label", "Abrir menú");

    button.setAttribute("aria-controls", "technicianSidebar");
...
function setSidebarState(open) {
    document.body.classList.toggle("technician-sidebar-open", open);

    document
        .getElementById("technicianMenuButton")
...
async function handleLogout(event) {
    ...
    try {
        ...
        window.alert("No fue posible cerrar la sesión. Intentá nuevamente.");
    }
}
...
function bindEvents() {
    ...
    const closeButton = document.getElementById("technicianSidebarClose");
    ...
    if (document.body.dataset.technicianLayoutLogout === "true") {
    ...
    document.querySelectorAll(".technician-nav-item").forEach((item) => {
        item.addEventListener("click", closeSidebar);
    });
}

function initializeTechnicianLayout() {
    document.body.classList.add("technician-layout-enabled");
