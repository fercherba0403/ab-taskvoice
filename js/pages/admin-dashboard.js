...
document.querySelectorAll("[data-coming-soon]").forEach((item) => {
    item.addEventListener(
        ...
        (event) => {
            ...
            showToast(`${item.dataset.comingSoon}: módulo pendiente.`);
        },
    );
});
...
newTaskButton.addEventListener("click", () => {
    window.location.href = "./tarea-nueva.html";
});
...
async function initialize() {
    ...
    sidebarUserRole.textContent = formatRole(profile.rol);

    sidebarAvatar.textContent = profile.nombre?.charAt(0)?.toUpperCase() ?? "U";

    welcomeMessage.textContent = `Hola ${profile.nombre}, este es el resumen de actividad.`;

    try {
        const dashboard = await getDashboardData(profile.organization_id);

        organizationName.textContent = dashboard.organizationName;
