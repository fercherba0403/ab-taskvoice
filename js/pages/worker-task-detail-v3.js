...
const photoGalleryButton = document.getElementById("photoGalleryButton");
...
const completedPhotosEmpty = document.getElementById("completedPhotosEmpty");
...
function updatePhotoControls() {
    const canEdit =
        currentAssignment?.estado === "en_progreso" && Boolean(currentExecutionId);

    const limitReached = currentPhotos.length >= 3;

    const disabled = !canEdit || limitReached || photoOperationBusy;

    photoCameraButton.disabled = disabled;

    photoGalleryButton.disabled = disabled;

    photoCounter.textContent = `${currentPhotos.length} / 3`;
}

async function buildPhotoCard(photo, editable) {
    const card = document.createElement("article");

    card.className = "worker-photo-card";

    const link = document.createElement("a");

    link.className = "worker-photo-link";
    ...
    try {
        const signedUrl = await createExecutionPhotoSignedUrl(photo.storage_path);

        if (!signedUrl) {
            throw new Error("URL de fotografía no disponible.");
        }
        ...
        image.className = "worker-photo-image";

        image.src = signedUrl;

        image.alt = "Fotografía del trabajo realizado";
        ...
        errorBox.className = "worker-photo-image-error";

        errorBox.textContent = "No se pudo cargar la vista previa";

        link.append(errorBox);
    }
    ...
    if (editable) {
        ...
        footer.className = "worker-photo-card-footer";

        const deleteButton = document.createElement("button");

        deleteButton.type = "button";

        deleteButton.className = "worker-photo-delete";

        deleteButton.textContent = "Eliminar";

        deleteButton.disabled = photoOperationBusy;

        deleteButton.addEventListener(
            "click",

            async () => {
                if (photoOperationBusy) {
                    return;
                }
                ...
                photoOperationBusy = true;

                updatePhotoControls();

                deleteButton.disabled = true;

                showWorkMessage("Eliminando fotografía...");

                try {
                    await removeExecutionPhoto(photo);

                    await refreshWorkPhotos();

                    showWorkMessage("Fotografía eliminada.", "success");
                } catch (error) {
                    console.error("Error eliminando fotografía:", error);

                    showWorkMessage(
                        error.message ?? "No fue posible eliminar la fotografía.",
                        "error",
                    );
                } finally {
                    photoOperationBusy = false;
...
async function renderPhotoCollection(
    ...
    for (const photo of photos) {
        const card = await buildPhotoCard(photo, editable);
...
async function refreshWorkPhotos() {
    if (!currentExecutionId) {
        ...
        await renderPhotoCollection(photoGrid, photoEmpty, currentPhotos, true);
    ...
    currentPhotos = await getExecutionPhotos(currentExecutionId);
...
async function renderCompletedPhotos(executionId) {
    if (!executionId) {
        completedPhotos.replaceChildren();

        completedPhotosEmpty.classList.remove("hidden");

        return;
    }

    const photos = await getExecutionPhotos(executionId);
...
async function handleSelectedPhoto(file, input) {
    try {
        if (!file) {
            return;
        }
        ...
        if (currentPhotos.length >= 3) {
            throw new Error("La intervención ya tiene el máximo de 3 fotografías.");
        }
        ...
        showWorkMessage("Subiendo fotografía...");

        await uploadExecutionPhoto(currentTask, currentExecutionId, file);

        await refreshWorkPhotos();

        showWorkMessage("Fotografía agregada correctamente.", "success");
    } catch (error) {
        console.error("Error cargando fotografía:", error);

        showWorkMessage(
            error.message ?? "No fue posible cargar la fotografía.",
            "error",
        );
    } finally {
        if (input) {
...
async function renderCompletedWork(execution) {
    ...
    await renderCompletedPhotos(execution.id);
