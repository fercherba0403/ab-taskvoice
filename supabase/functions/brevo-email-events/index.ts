...
const TASKVOICE_NOTIFICATION_TAG =
...
function parseNumericTimestamp(
    value: unknown
): number | null {
...
function extractNotificationId(
    ...
    for (const tag of tags) {
        ...
        const notificationId =
            normalizeInteger(
                normalized.slice(
                    TASKVOICE_NOTIFICATION_TAG.length
                )
            );
...
async function recordEvent(
    ...
    if (
        ...
        throw new Error(
            'Payload transaccional de Brevo inválido.'
        );
    ...
    const {
        ...
        'record_brevo_email_event',
        {
            p_notification_id:
                extractNotificationId(payload),
            ...
            p_reason:
                resolveReason(payload),

            p_clicked_url:
                resolveClickedUrl(payload),
    ...
    if (error) {
        ...
        throw new Error(
            `No se pudo registrar el evento: ${error.message}`
        );
    ...
    if (!isJsonRecord(data)) {
        ...
        throw new Error(
            'La RPC devolvió una respuesta inválida.'
        );
...
Deno.serve(
    async (
        ...
        if (webhookSecret.length < 32) {
            ...
            console.error(
                'brevo-email-events: BREVO_WEBHOOK_SECRET no configurado.'
            );
        ...
        if (
            ...
            console.error(
                'brevo-email-events: configuración de Supabase incompleta.'
            );
