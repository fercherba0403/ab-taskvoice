...
type ClaimResult = "claimed" | "existing" | "busy" | "attempts_exhausted";
...
function normalizeStoredError(context: string, error: unknown) {
...
function claimErrorResponse(message: string) {
  const normalized = message.toLowerCase();

  if (normalized.includes("reporte inexistente")) {
    return jsonResponse({ error: "Reporte inexistente." }, 404);
  }
...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    const groqApiKey = getRequiredEnvironmentVariable("GROQ_API_KEY");
    ...
    if (!Number.isSafeInteger(reportId) || reportId <= 0) {
      return jsonResponse({ error: "report_id invalido." }, 400);
    }
    ...
    if (claimError) {
      console.error("transcribe-execution-report claim:", claimError.message);
    ...
    if (claim.claim_result === "existing") {
      return jsonResponse({
        ...
        transcription: claim.existing_transcription ?? "",
      });
    }
    ...
    if (claim.claim_result !== "claimed") {
      return jsonResponse({ error: "Estado de reclamo no reconocido." }, 500);
    }
    ...
    if (audioError || !audioBlob) {
      ...
      await markAttemptFailed(adminClient, activeClaim, storedError);
      activeClaim = null;

      return jsonResponse({ error: "No fue posible obtener el audio." }, 500);
    }
    ...
    const fileName = claim.audio_path.split("/").pop() ?? "audio.webm";
    ...
    formData.append(
      ...
      [
        ...
        claim.task_title ? `Tarea: ${claim.task_title}.` : "",
        claim.ticket_number ? `Ticket: ${claim.ticket_number}.` : "",
      ]
        .filter(Boolean)
        .join(" "),
    );
    ...
    if (!groqResponse.ok) {
      const errorText = (await groqResponse.text()).slice(0, 500);
    ...
    try {
      transcriptionResult = (await groqResponse.json()) as JsonObject;
    } catch (error) {
      await markAttemptFailed(
        adminClient,
        activeClaim,
    ...
    console.error("transcribe-execution-report:", getErrorMessage(error));
    ...
    return jsonResponse({ error: "Error interno de transcripcion." }, 500);
  }
});
