...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const roleConfiguration = resolveProfileConfiguration(body);
    ...
    const roles = [...roleConfiguration.roles];
    ...
    if (createError || !createData.user) {
      console.error("Auth createUser:", createError);

      return jsonResponse(
        {
          error: createError?.message ?? "No fue posible crear el usuario.",
        },
        400,
      );
    }
    ...
    if (metadataError) {
      console.error("Error app_metadata:", metadataError);
      ...
      return jsonResponse(
        {
          error: "No fue posible configurar los permisos del usuario.",
        },
        500,
      );
    }
    ...
    if (insertError) {
      ...
      return jsonResponse(
        {
          error: "No fue posible crear el perfil del usuario.",
        },
        500,
      );
    }
    ...
    if (rolesInsertError) {
      console.error("Error creando user_roles:", rolesInsertError);
      ...
      return jsonResponse(
        {
          error: "No fue posible configurar los perfiles del usuario.",
        },
        500,
      );
    }
